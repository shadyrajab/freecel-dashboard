...
class Rankings:
    ...
    def full_ranking(self):
        altas_e_portabilidade = pd.merge(self['altas'], self['portabilidade'], on='consultor', suffixes=('_df1', '_df2'), how='outer').fillna(0)
        altas_e_portabilidade['volume'] = altas_e_portabilidade['volume_df1'] + altas_e_portabilidade['volume_df2']
        altas_e_portabilidade['receita'] = altas_e_portabilidade['receita_df1'] + altas_e_portabilidade['receita_df2']
        altas_e_portabilidade.drop(['clientes_df1', 'clientes_df2', 'receita_df1', 'receita_df2', 'volume_df1', 'volume_df2'], axis=1, inplace=True)

        altas_e_portabilidade = altas_e_portabilidade.rename(columns={'volume': 'Volume Altas', 'receita': 'Receita Altas'})
        ...
        altas_geral = pd.merge(altas_e_portabilidade, geral, on='consultor', how='outer')
